...
create or replace procedure ods.upsert_segments_from_stage()
language plpgsql
as $$
begin
...
create or replace procedure ods.delete_segments_from_stage()
language plpgsql
as $$
begin
    update ods.segments o
    set
...
create or replace procedure ods.apply_segments_from_stage()
language plpgsql
as $$
declare
...
begin
    ...
    call ods.upsert_segments_from_stage();

    raise notice 'ods.upsert_segments_from_stage duration: %',
        clock_timestamp() - v_step_started_at;


    v_step_started_at := clock_timestamp();

    call ods.delete_segments_from_stage();

    raise notice 'ods.delete_segments_from_stage duration: %',
        clock_timestamp() - v_step_started_at;


    raise notice 'ods.segments applied. rows = %, active rows = %, deleted rows = %',
        (select count(*) from ods.segments),
        (select count(*) from ods.segments where is_deleted = false),
        (select count(*) from ods.segments where is_deleted = true);
end;
$$;